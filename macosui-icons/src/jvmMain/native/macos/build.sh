#!/bin/bash
# Compiles NucleusSFSymbolBridge.m into per-architecture dylibs (arm64 + x86_64).
# The outputs are placed in the JAR resources so they ship with the library.
#
# Prerequisites: Xcode command-line tools (clang), JDK with JNI headers.
# Usage: ./build.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SRC="$SCRIPT_DIR/NucleusSFSymbolBridge.m"
...
clang -arch arm64 "${COMMON_FLAGS[@]}" \
    -o "$OUT_DIR_ARM64/libnucleus_sfsymbols.dylib" "$SRC"
strip -x "$OUT_DIR_ARM64/libnucleus_sfsymbols.dylib"

clang -arch x86_64 "${COMMON_FLAGS[@]}" \
    -o "$OUT_DIR_X64/libnucleus_sfsymbols.dylib" "$SRC"
strip -x "$OUT_DIR_X64/libnucleus_sfsymbols.dylib"

echo "Built per-architecture dylibs:"
ls -lh "$OUT_DIR_ARM64/libnucleus_sfsymbols.dylib"
ls -lh "$OUT_DIR_X64/libnucleus_sfsymbols.dylib"
